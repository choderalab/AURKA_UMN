...
alignment_selection_dsl = '(resSeq >= 217) and (resSeq <= 387) and (name CA)'
alignment_reference_indices = reference.topology.select(alignment_selection_dsl)

rmsd_selection_dsl = '(resSeq >= 123) and (resSeq <= 210) and (name CA)'
...
for project in projects:
    for run in range(nruns):
        ...
        if rank == 0:
            ...
            output_filename = os.path.join(output_basepath, '%s-run%d-nloop-rmsd_1OL5.npy' % (project, run))
