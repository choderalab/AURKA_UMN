...
parser.add_argument('--verbose', required=False, action='store_true', dest='verb',
                    help='use to make this verbose')
...
def protein_prep(input_file, output_file, cap, pH=7.4, fillsidechains=True, fillloops=True,
                 noepik=False, rehtreat=True, max_states=32, tolerance=0, verbose=False):

    # Locate PrepWizard executable
    if verbose: print('finding prepwizard executable')
    prepwiz_path = os.path.join(os.environ['SCHRODINGER'], 'utilities', 'prepwizard')

    # Normalize paths
    if verbose: print('normalizing paths')
    ...
    if not os.path.exists(working_dir):
        if verbose: print('creating working directory...')
    ...
    with utils.temporary_cd(working_dir):
        if verbose: print('Running prepwizard')
...
def openmm_clean(input_file, output_file, solvate=False, verbose=False):
    ...
    if verbose: print('Initializing forcefield')
    ...
    if verbose: print('discarding unwanted molecules')
    model = discard_organic(model, verbose=False)

    # Add waters in a cubic box
    if solvate == True:
        if verbose: print('solvating system')
    ...
    if verbose: print('minimizing system')
    ...
    # Print PDB
    if verbose: print('writing PDB file')
...
if __name__ == '__main__':
    ...
    protein_prep(file_name, output, cap, pH=ph, verbose=verbosity)
    openmm_clean(file_name, output, solvate=True, verbose=verbosity)
