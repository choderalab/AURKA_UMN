...
def some_traj_things():
    ...
    return trajectories, longest_traj, time, files, sim_num, total_sim_time, sim_time

### THE FIRST COUPLE PLOTS ARE ONLY CONDUCTED ON THE LONGEST SIM.

### RMSD PLOT
def longest_rmsd(longest_traj, time):
...
def longest_dssp(longest_traj, time):
...
### DFG PLOT
def grouped_dfg_dihedral(trajectories):

    with open('Aloop_def.json', 'r') as fp:
        Aloop = json.load(fp)

    [Src_dihedral, non_flat_version] = kinalysis.DFG_dihedral(trajectories, Aloop[protein])
...
def grouped_proline_dihedral(trajectories):
    with open('Aloop_def.json', 'r') as fp:
        Aloop = json.load(fp)

    [Src_dihedral, non_flat_version] = kinalysis.proline_dihedral(trajectories, Aloop[protein])

    #Save dihedral
    np.save('proline_dihedral_%s.npy'%protein,Src_dihedral)
    ...
    plt.tight_layout()
    plt.savefig('proline_dihedral.png')
    plt.clf()
    return Aloop
...
def byruns_dfg_dihedral(files, Aloop):
    [dihedral] = kinalysis.DFG_dihedral_byrun(files, Aloop[protein])
    ...
    [lines, _ ] = kinalysis.DFG_dihedral(firstframes, Aloop[protein])
...
def byruns_proline_dihedral(files, Aloop=None):
    if Aloop is None:
        with open('Aloop_def.json', 'r') as fp:
            Aloop = json.load(fp)
    [dihedral] = kinalysis.proline_dihedral_byrun(files, Aloop[protein])

    #Save dihedral
    np.save('proline_dihedral_separate_%s.npy'%protein,dihedral)

    # Calculate dihedral at the first frame of clone0 of each run
    path_base = files.split('*')[0]
    clone0_trajectories = dataset.MDTrajDataset("%s/*clone0.h5" % path_base )
    firstframes = [traj[0] for traj in clone0_trajectories]
    [lines, _ ] = kinalysis.proline_dihedral(firstframes, Aloop[protein])
    ...
    DFG_in = np.where(line_rotate > -1.5)
    DFG_out = np.where(line_rotate < -1.5)
    ...
    # Plot histogram with special seaborn sauce
    try:
        sns.distplot(dihedral_in_flattened, color="m",label="P282 cis (%s) " %len(DFG_in[0]) )
    except:
        pass
    try:
        sns.distplot(dihedral_out_flattened, color="r",label="P282 trans (%s) " %len(DFG_out[0]) )
    except:
        pass
    ...
    try:
        ...
        plt.savefig('%s_proline_dihedral_inout_lines.png'%protein)
        plt.clf()
    except:
        pass
...
#make PDF summary
def make_pdf_summary(sim_num, total_sim_time, sim_time):
...
if __name__ == "__main__":
    trajectories, longest_traj, time, files, sim_num, total_sim_time, sim_time = some_traj_things()
    longest_rmsd(longest_traj, time)
    #longest_dssp(longest_traj, time)
    #Aloop = grouped_dfg_dihedral(trajectories)
    #_ = grouped_proline_dihedral(trajectories)
    #byruns_dfg_dihedral(files, Aloop)
    #byruns_proline_dihedral(files)
    #shukla(trajectories)
    #make_pdf_summary(sim_num, total_sim_time, sim_time)
