...
def alpha_distances(traj, residue_pair):
    ...
    atom1 = short_traj.topology.select("residue %s and name == 'CB' and chainid 0" % residue_pair[0])
    atom2 = short_traj.topology.select("residue %s and name == 'CZ' and chainid 0" % residue_pair[1])
    ...
    for pair in range(len(res_pairs['AURKA'])):
        ...
        for traj_in in trajectories:
            if len(traj_in) > 400:
                ...
                len_list.append(len(traj_in))
        np.save('../data/distances/288-180/distances_CHARMM_AURKA_%s-pair%s-%s.npy' % (condition, pair_list[0], pair_list[1]),
                dist_list)
