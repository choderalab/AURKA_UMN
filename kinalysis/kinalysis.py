...
def DFG_dihedral(trajectories,res_DFG):

    dihedral = []

    for traj in trajectories:
        a273 = traj.top.residue(res_DFG[0]-1)
        d274 = traj.top.residue(res_DFG[0])
        ...
        for atom in d274.atoms:
            ...
            if atom.name == 'CG':
                aspcg = atom.index
        def_DFG = [alacb, alaca, aspca, aspcg]
        try:
    ...
    return [flattened_dihedral, dihedral]

def proline_dihedral(trajectories,res_DFG):

    dihedral = []

    for traj in trajectories:
        a281 = traj.top.residue(res_DFG[0]+7)
        p282 = traj.top.residue(res_DFG[0]+8)
        for atom in a281.atoms:
            if atom.name == 'CA':
                alaca = atom.index
            if atom.name == 'C':
                alac = atom.index
        for atom in p282.atoms:
            if atom.name == 'CA':
                proca = atom.index
            if atom.name == 'N':
                pron = atom.index
        def_DFG = [alaca, alac, pron, proca]
        try:
            dihedral.append(md.compute_dihedrals(traj,[def_DFG]))
        except:
            continue
...
def DFG_dihedral_byrun(files,res_DFG):
...
def proline_dihedral_byrun(files,res_DFG):
